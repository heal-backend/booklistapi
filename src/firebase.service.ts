...
async changeData(): Promise<void> {
    ...
    if (a.val().apple.name === "apple") {
        await Promise.all([
            ref.child("apple").child('name').set("changedApple"),
            ref.child("banana").child('name').set("changedBanana"),
            ref.child("peach").child('name').set("changedPeach"),
            ref.child("melon").child('name').set("changedMelon")
        ])
        return
    }
    await ref.set(datas)
